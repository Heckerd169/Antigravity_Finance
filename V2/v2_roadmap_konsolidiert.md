...
| PF-6 | Ein Import mit mehr als vier neuen Zahlungen reißt das 8-Sekunden-Zeitlimit | Fehler | **ja** | 🟡 | **In Arbeit, v2-30.** Der Nutzer konnte am 27.08.2026 seine beiden Monatsabzüge nicht importieren. **Die Dateien sind einwandfrei** — der Import überschreitet den `statement_timeout` der Rolle `authenticated` (**8 s**; `postgres`/`service_role` haben **keinen**, weshalb jeder MCP-Trockenlauf den Fehler verfehlt). Gemessen kostet eine **neue** Zahlung **1.995 ms**; A/B mit identischem Payload: 545 ms als Dienst-Rolle gegen 9.973 ms als `authenticated`, **Faktor 18**. Edge-Logs zeigen die Signatur eines Timeouts, nicht eines Datenfehlers: vier Fehlschläge mit 8.233–9.249 ms und Status 500, zwei Erfolge mit 629 und 767 ms — **dazwischen liegt nichts**. **Duplikate und interne Überträge überspringen die Rechnung** (`IF v_was_inserted AND NOT v_is_internal`), deshalb ging eine alte Datei problemlos durch und deshalb hat **kein Wächter** angeschlagen: jede Zahl ist richtig, sie kommt nur nicht mehr an (LL-28/LL-29). **Eine Ursache steht fest:** `history_match` wird je Karte aufgerufen, aber ihre teure Stufe-1-Abfrage hängt am **Fragment**, nicht an der Karte — bei 28 Karten läuft **28-mal dieselbe Abfrage** (belegt: 28× = 255 ms gegen gemessene 264–278 ms). **Aber das sind nur ~13 % der Import-Kosten**; ~1.310 ms je Zahlung sind noch nicht lokalisiert, weshalb v2-30 eine eigene Mess-Phase vor der Bau-Phase hat. Diagnose: `V2/befunde_2026-08-27_import-zeitlimit.md`. **Nicht mit `PF-3` verwechseln** — die Policy-Umstellung ist hier gemessen wirkungslos. |
